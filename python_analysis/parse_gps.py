...
def load_by_user(limit =-1):
    ...
    dirs = glob.glob("/nfs/beirut1/userstudy/2nd_round/201210/**/psmn-20*/device_info.zip")
...
#    f = open("user_loc_trends.txt", "w")
    for user, loc_timeline in user_locs.iteritems():
        ...
        for loc in loc_timeline:
            ...
            loc_number = 10 
#            print loc_code, user_top_locs[user]
            if loc_code in user_top_locs[user]:
                loc_number = user_top_locs[user].index(loc_code)
             
#            print >>f, user, loc[0], loc[-1], loc_code, loc_number
#            print >>f, user, loc[-1], 0, loc_code, loc_number

            loc_timestream[user].append(GpsAttribute(user, loc[0], loc[-1], loc_number))
        loc_timestream[user].sort(key=operator.attrgetter("time"))
...
if __name__ == "__main__":
    ...
    gps_timeline = load_by_user(100)

    timestamp_adjustor = 3600 * 24 * 7
    timeline = timeline.TimeLine("data_by_hour_gps")
    flow_timeline = timestream.load_timeline(100)
